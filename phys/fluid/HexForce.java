 ...
 */
final class HexForce extends Force {
  ...
  public final void apply(final int radius, final Space2D before, final Space2D after,
                          final int x, final int y) {
    final int in =
      before.get(x - 1, y - 1) & DR
      | before.get(x + 1, y - 1) & DL
      | before.get(x - 1, y) & R
      | before.get(x + 1, y) & L
      | before.get(x - 1, y + 1) & UR
      | before.get(x + 1, y + 1) & UL;
