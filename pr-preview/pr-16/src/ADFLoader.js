...
function toothPose(tooth) {
  ...
  let z = tooth.faDir ? v3(tooth.faDir).normalize() : new Vector3(0, 0, 1);
  const towardCrown = crown.clone().sub(origin);
  if (towardCrown.lengthSq() > 1e-12 && towardCrown.dot(z) < 0) z.negate();
  let y = tooth.faNormal ? v3(tooth.faNormal).normalize() : new Vector3(0, 1, 0);
  if (Math.abs(y.dot(z)) > 0.95) y = new Vector3(0, 1, 0);
...
function roundedRect(w, d, t) {
  ...
  s *= perim;
  const q = Math.PI / 2;
  if (s < straightW) return [ -hw + r + s, hd ];
  s -= straightW;
  if (s < r * q) {
    const a = s / r;
  ...
  s -= r * q;
  if (s < straightD) return [ hw, hd - r - s ];
  s -= straightD;
  if (s < r * q) {
    const a = s / r;
...
export function createToothGeometry(kind, width, depth, height) {
  ...
  cy /= segs;
  cz /= segs;
  if (kind === 'molar' || kind === 'premolar') cz -= height * 0.04;
...
function featureColor(name) {
  ...
  if (n.includes('facc') || n.includes('ridge')) return 0x5ad0ff;
  if (n.includes('buccal')) return 0xff8a4a;
  if (n.includes('lingual')) return 0x7dffb3;
  if (n.includes('groove')) return 0xd97bff;
  if (n.includes('tip')) return 0xffe066;
  return 0xcfd8e3;
}
...
function buildJaw(jaw, options) {
  ...
  for (const tooth of jaw.teeth) {
    ...
    const cloud = [];
    for (const set of tooth.ipPoints) cloud.push(...set);
    if (tooth.cejPoints.length) cloud.push(...tooth.cejPoints);
    if (cloud.length) {
      const pts = new Points(
