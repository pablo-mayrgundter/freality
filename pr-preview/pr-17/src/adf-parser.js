...
function peekName(bytes, pos, end) {
  if (pos >= end || !IDENT.test(String.fromCharCode(bytes[pos]))) return null;
  let i = pos;
  while (i < end && IDENT.test(String.fromCharCode(bytes[i]))) i++;
...
function parseObjectBody(bytes, pos, end) {
  ...
  while (pos < end && bytes[pos] !== 0x7d) {
    ...
    pos += name.length + 1;
    if (pos + 4 > end) throw new ADFParseError(`truncated size for ${name}`, pos);
    const size = viewAt(bytes, pos).getUint32(0, true);
    pos += 4;
    if (pos + size > end) throw new ADFParseError(`field ${name} overruns`, pos);
...
function decodeValue(bytes) {
  ...
  if (bytes[start] === 0x7b && bytes[n - 1] === 0x7d) {
    try {
      ...
      if (pos === n) return items.length === 1 ? items[0] : items;
    } catch {
      // fall through
    }
  }
  ...
  if (peekName(bytes, 0, n)) {
    try {
      ...
      if (body.pos === n) return body.value;
    } catch {
  ...
  if (n >= 8 && n % 4 === 0 && n <= 8 + 4096 * 12) {
    ...
    if (count > 0 && 4 + count * 4 === n) {
      const idx = [];
      for (let i = 0; i < count; i++) idx.push(dv.getInt32(4 + i * 4, true));
      return { ints: idx };
    }
  }
...
export function i32(value, fallback = 0) {
  ...
  if (typeof value === 'object' && typeof value.i32 === 'number') return value.i32;
  return fallback;
}
...
export function f32(value, fallback = 0) {
  if (value == null) return fallback;
  if (typeof value === 'number') return value;
  if (typeof value === 'object' && typeof value.f32 === 'number') return value.f32;
  return fallback;
}
...
export function vec3(value) {
  ...
  if (value.vec3) return value.vec3;
  if (value.translation) return vec3(value.translation);
  return null;
}
...
export function quatWxyz(value) {
  ...
  if (!value) return null;
  if (value.vec4) return value.vec4;
  if (value.rotation) return quatWxyz(value.rotation);
  return null;
}

export function transformOf(obj) {
  if (!obj || obj.__type !== 'object') return null;
...
function collectNamed(obj, name, out = []) {
  ...
  if (obj.__type === 'object') {
    if (Object.prototype.hasOwnProperty.call(obj, name)) {
      for (const v of asList(obj[name])) out.push(v);
    }
    ...
    for (const item of obj) collectNamed(item, name, out);
  }
  return out;
}
...
function floatArrayPoints(value) {
  if (!value) return [];
  if (value.points) return value.points;
  return [];
}

function splinePoints(splineObj) {
  if (!splineObj || splineObj.__type !== 'object') return [];
  const handlers = asList(splineObj.CVHandler);
  const pts = [];
  for (const h of handlers) {
    const xf = transformOf(h.Transform) || transformOf(h);
    const t = xf?.translation || vec3(h.Transform);
    if (t) pts.push(t);
  }
  return pts;
}
...
function extractTooth(obj, jawName) {
  ...
  if (facc && facc.FACCCurveData) facc = facc.FACCCurveData;
  ...
  for (const key of obj.__keys || []) {
    for (const item of asList(obj[key])) {
      if (!item || item.__type !== 'object') continue;
  ...
  for (const blob of vtxBlobs) {
    ...
    if (pts.length) ipPoints.push(pts);
  }
  let maxVertexId = 0;
  for (const blob of collectNamed(obj, 'newvtxIndices')) {
    const ids = blob?.ints || [];
    for (const i of ids) if (i > maxVertexId) maxVertexId = i;
  }
...
function extractGingiva(jawObj) {
  ...
  for (const h of handlers) {
    ...
    if (!xf) continue;
    if (n.startsWith('Lab+')) labH.push([n, xf.translation]);
    else if (n.startsWith('Lin+')) linH.push([n, xf.translation]);
  }
