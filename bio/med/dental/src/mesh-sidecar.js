...
export function parseMeshSidecar(input) {
  ...
  if (magic !== 'ADFM') throw new Error('not an ADF mesh sidecar');
  const version = dv.getUint32(4, true);
  if (version !== 1) throw new Error(`unsupported mesh sidecar version ${version}`);
  ...
  for (let m = 0; m < count; m++) {
    ...
    const min = [0, 1, 2].map((i) => dv.getFloat32(pos + 16 + 4 * i, true));
    const max = [0, 1, 2].map((i) => dv.getFloat32(pos + 28 + 4 * i, true));
    pos += 40;
